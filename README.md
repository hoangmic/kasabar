...
- 🎯 Entry points to execute your program using `python -m <kasabar>` or `$ kasabar` with basic CLI argument parsing.
...
---
# kasabar Flask Application

Awesome kasabar created by hoangmic
...
```bash
git clone https://github.com/hoangmic/kasabar kasabar
cd kasabar
make install
```
...
```bash
kasabar create-db   # run once
kasabar populate-db  # run once (optional)
kasabar add-user -u admin -p 1234  # ads a user
kasabar run
```
