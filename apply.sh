...
original_author="hoangmic"
original_name="kasabar"
original_urlname="kasabar"
original_description="Awesome kasabar created by hoangmic" 
...
rm -rf "${name}"
rm -rf tests
cp -R ${TEMPLATE_DIR}/kasabar "${name}"
...
then
    ...
    echo "then run 'kasabar' or 'python -m kasabar'"
else
    poetry install
    echo "Applied Flask template"
    echo "Ensure you activate your env with 'poetry shell'"
    echo "then run 'kasabar' or 'python -m kasabar' or 'poetry run kasabar'"
fi
